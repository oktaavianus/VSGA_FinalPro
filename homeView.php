...
<body>
  <nav class="navbar navbar-expand-lg navbar-dark bg-dark shadow-lg fixed-top">
    <div class="container text-center">
      <a class="navbar-brand md-auto mb-2 mb-lg-0" href="./homeView.php">Eat Here.</a>
  ...
  <div class="container-fluid menus" id="menus"style="padding-top: 70px;">
    <div class="container text-center">
      <h3 class="display-3">Our Menus</h3>
      ...
      <div class="row mt-5">
        ...
        $queryProduct = mysqli_query($conn, "SELECT * FROM products JOIN category ON products.category=category.id");
        while($product = mysqli_fetch_assoc($queryProduct)) :
        // echo $queryProduct['imgUrl']
        ?>
        <div class=" col-sm-6 col-md-4 mb-3">
          <div class="card menus">
            <!-- Image URL -->
            <img src="<?= $product['imgUrl']?>" class="card-img-top image-src" alt="meatplatter">
            <!-- /Image URL -->
            <div class="card-body">
              <!-- Category -->
              <div class="card-text display-7">
                <p><?php echo $product['categoryName']?></p>
              </div>
              <!-- /Category -->
              <!-- Menu Name -->
              <h5 class="card-title display-3"><?php echo $product['nama']?></h5>
              <!-- Menu Description -->
              <p class="card-text"><?php echo $product['description']?></p>
              <!-- Menu Price -->
              <div class="card-text display-6">
                <h6 class="display-6">Rp <?php echo number_format($product['price'], 0, ',', '.');?>,-</h6>
              </div>
              <!-- /Menu Price -->
              <div class="row mt-5 list-menu">
                <div class="col-6">
                  <button type="button" class="btn btn-primary btn-lg btn-block price" data-price="<?= $product['price']?>">Add Cart</button>
                </div>
                <div class="col-6">
                  <button type="button" class="btn btn-danger btn-lg btn-block price" data-price="<?= $product['price']?>">Remove Cart</button>
                </div>
              </div>
            </div>
          </div>
